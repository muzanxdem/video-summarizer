...
# Sidebar UI
st.sidebar.title("Intelvisual: Video Summarizer")
st.sidebar.write("Upload a video or audio file to transcribe, summarize, and ask questions.")

# Input option (File Upload Only)
uploaded_file = st.sidebar.file_uploader("Upload a video/audio file", type=["mp4", "mkv", "avi", "wav", "mp3"])

# Main interface
st.title("Video Summarizer and Q&A")
st.write("This app transcribes, summarizes, and enables you to ask questions about uploaded video/audio files.")

if st.sidebar.button("Summarize"):
    if not uploaded_file:
        st.error("Please upload a valid video or audio file.")
    else:
        try:
            # Step 1: Extract audio from uploaded file
